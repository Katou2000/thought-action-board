...
d.quickTasks=Array.isArray(d.quickTasks)?d.quickTasks:[];d.quickTasks=d.quickTasks.filter(t=>t&&typeof t==="object").map(t=>({id:t.id||uid(),title:String(t.title||"").trim(),boardId:d.boards.some(b=>b.id===t.boardId)?t.boardId:d.selectedBoardId,plannedDate:typeof t.plannedDate==="string"&&/^\d{4}-\d{2}-\d{2}$/.test(t.plannedDate)?t.plannedDate:localDate(),createdAt:Number(t.createdAt)||Date.now(),completed:!!t.completed,completedAt:typeof t.completedAt==="string"?t.completedAt:""})).filter(t=>t.title);const quickLogCutoff=Date.now()-14*86400000;d.quickTaskLog=Array.isArray(d.quickTaskLog)?d.quickTaskLog:[];d.quickTaskLog=d.quickTaskLog.filter(t=>t&&t.title&&Date.parse(t.completedAt)>=quickLogCutoff).map(t=>({id:t.id||uid(),title:String(t.title),boardId:d.boards.some(b=>b.id===t.boardId)?t.boardId:d.selectedBoardId,plannedDate:typeof t.plannedDate==="string"?t.plannedDate:"",completedAt:t.completedAt}));
...
d.quickMemos=Array.isArray(d.quickMemos)?d.quickMemos:[];d.quickMemos.forEach(m=>{m.id=m.id||uid();m.title=m.title||"";m.content=m.content||"";m.pinned=!!m.pinned;m.createdAt=m.createdAt||m.updatedAt||Date.now();m.updatedAt=m.updatedAt||m.createdAt});if(d.selectedQuickMemoId&&!d.quickMemos.some(m=>m.id===d.selectedQuickMemoId))d.selectedQuickMemoId=d.quickMemos[0]?.id||null;
...
ensureCurrentData();
function save(){mutationSerial++;if(pendingUndo&&!undoRunning&&mutationSerial!==pendingUndo.serial)clearLightUndo();try{localStorage.setItem(STORAGE_KEY,JSON.stringify(data));storageUsage()}catch(e){alert("保存容量がいっぱいの可能性があります。画像やゴミ箱を整理してください。");return}window.taskKanrinnerCloudSync?.onLocalSaved()}
...
function routineCreatedKey(r){return getRoutineLogicalDate(new Date(r.createdAt||Date.now()))}
function routineScheduledOn(r,dateKey=getRoutineLogicalDate()){if(dateKey<routineCreatedKey(r))return false;const day=new Date(dateKey+"T00:00:00").getDay();if(r.rule==="daily")return true;if(r.rule==="weekdays")return day>=1&&day<=5;if(r.rule==="custom")return (r.weekdays||[]).includes(day);return true}
function routineApplicable(r,dateKey=getRoutineLogicalDate()){return !r.paused&&routineScheduledOn(r,dateKey)}
function routineDoneOn(r,dateKey=getRoutineLogicalDate()){return !!(r.logs&&r.logs[dateKey])}
function routineMark(r,done,dateKey=getRoutineLogicalDate()){r.logs=r.logs||{};if(done)r.logs[dateKey]=true;else delete r.logs[dateKey];r.updatedAt=Date.now()}
function routineWeekStats(r,anchor=new Date(getRoutineLogicalDate()+"T12:00:00")){const x=new Date(anchor);const day=(x.getDay()+6)%7;x.setDate(x.getDate()-day);let total=0,done=0;for(let i=0;i<7;i++){const d=new Date(x);d.setDate(x.getDate()+i);const k=localDate(d);if(routineScheduledOn(r,k)){total++;if(routineDoneOn(r,k))done++}}return{done,total}}
function routineStreak(r){let streak=0;const d=new Date(getRoutineLogicalDate()+"T12:00:00");for(let i=0;i<365;i++){const x=new Date(d);x.setDate(d.getDate()-i);const k=localDate(x);if(!routineScheduledOn(r,k))continue;if(routineDoneOn(r,k))streak++;else break}return streak}
function routineStats(dateKey=getRoutineLogicalDate()){const applicable=data.routines.filter(r=>routineApplicable(r,dateKey));const done=applicable.filter(r=>routineDoneOn(r,dateKey));const allTracked=data.routines.filter(r=>!r.paused);return{applicable,done,total:applicable.length,percent:applicable.length?Math.round(done.length/applicable.length*100):0,maxStreak:allTracked.length?Math.max(...allTracked.map(r=>routineStreak(r)),0):0}}
...
function applyTheme(){
  ...
  const nav=d?{todayTasks:"⚡ 今日やること",routine:"🧪 ルーティンタスク",calendar:"✦ カレンダー",home:"💎 ホーム",shortcuts:"☆ ピン留め",archive:"💥 完了したこと",templates:"🧩 テンプレート",freeboard:"🧠 自由帳",memo:"🎮 メモ",boards:"📁 ボード",builder:"🧱 ブロックビルドブレイカー",achievedGoals:"🏆 達成した目標",trash:"☠️ ゴミ箱"}:{todayTasks:"☀ 今日やること",routine:"🔁 ルーティンタスク",calendar:"📅 カレンダー",home:"🏠 ホーム",shortcuts:"☆ ピン留め",archive:"✅ 完了したこと",templates:"🧩 テンプレート",freeboard:"🗒 自由帳",memo:"📝 メモ",boards:"📁 ボード",builder:"🧭 目標設計",achievedGoals:"🏆 達成した目標",trash:"🗑 ゴミ箱"};
  Object.entries(nav).forEach(([k,v])=>{const b=E.mainNav.querySelector(`[data-nav-key="${k}"]`);if(b)b.textContent=v});
  renderNav();editMode();
}
const views=["todayTasks","routine","calendar","home","shortcuts","boards","board","archive","templates","freeboard","memo","builder","achievedGoals","trash"];
function syncViewState(view){views.forEach(v=>{E[v+"View"]?.classList.toggle("hidden",v!==view);E[v+"Button"]?.classList.toggle("active",v===view)});E.boardsButton?.classList.toggle("active",view==="boards"||view==="board")}
function show(view){data.view=view;save();syncViewState(view);renderSidebar();renderView()}
function showTransient(view){data.view=view;syncViewState(view);renderSidebar();renderView()}
function renderView(){({todayTasks:renderToday,routine:renderRoutine,calendar:renderCalendar,home:renderHome,shortcuts:renderShortcuts,boards:renderBoards,board:renderBoard,archive:renderArchive,templates:renderTemplates,freeboard:renderFreeboard,memo:renderMemos,builder:renderBuilder,achievedGoals:renderAchieved,trash:renderTrash}[data.view]||(()=>{}))()}
function renderAll(){applyTheme();renderNav();renderSidebar();syncViewState(data.view);renderView()}
...
function renderBoards(){
  ...
  data.boards.forEach(b=>{
    const row=document.createElement("article");row.className="board-overview-row";
...
function renderRoutineCard(r){
  const today=getRoutineLogicalDate(),done=routineDoneOn(r,today),active=routineApplicable(r,today),week=routineWeekStats(r),streak=routineStreak(r),status=r.paused?"休止中":done?"今日クリア":active?"今日の対象":"今日は対象外";
...
function routineReflectionTarget(r,dateKey){if(!routineScheduledOn(r,dateKey))return false;const today=getRoutineLogicalDate();/* pausedAt履歴がない既存形式では、休止は今日以降だけ除外し、過去ログを失わない。 */return !(r.paused&&dateKey>=today&&!routineDoneOn(r,dateKey))}
function routineDayReflection(dateKey,filterId=routineCalendarFilterId){const candidates=filterId==="all"?data.routines:data.routines.filter(r=>r.id===filterId),targets=candidates.filter(r=>routineReflectionTarget(r,dateKey)),done=targets.filter(r=>routineDoneOn(r,dateKey));return{targets,done,total:targets.length,doneCount:done.length,percent:targets.length?Math.round(done.length/targets.length*100):0}}
function routineReflectionStreaks(filterId=routineCalendarFilterId){const routines=filterId==="all"?data.routines:data.routines.filter(r=>r.id===filterId);if(!routines.length)return{current:0,longest:0};const today=new Date(getRoutineLogicalDate()+"T00:00:00"),limit=new Date(today);limit.setDate(limit.getDate()-3660);const created=routines.map(r=>new Date(routineCreatedKey(r)+"T00:00:00")).filter(d=>!Number.isNaN(d.getTime())),earliest=created.length?new Date(Math.max(limit.getTime(),Math.min(...created.map(d=>d.getTime())))):limit;let longest=0,run=0;for(const d=new Date(earliest);d<=today;d.setDate(d.getDate()+1)){const s=routineDayReflection(localDate(d),filterId);if(!s.total)continue;if(s.doneCount===s.total){run++;longest=Math.max(longest,run)}else run=0}let current=0;for(const d=new Date(today);d>=earliest;d.setDate(d.getDate()-1)){const s=routineDayReflection(localDate(d),filterId);if(!s.total)continue;if(s.doneCount===s.total)current++;else break}return{current,longest}}
function renderRoutineDayDetail(){const key=routineReflectionDate,s=routineDayReflection(key),future=key>getRoutineLogicalDate(),date=new Date(key+"T00:00:00"),heading=`${date.getMonth()+1}月${date.getDate()}日`;E.routineDayDetail.innerHTML=`<div class="routine-day-detail-head"><div><h4>${heading}</h4><p>${future?"予定されている対象":"その日の対象と実績"}</p></div><strong>${future?`${s.total}件予定`:`${s.doneCount} / ${s.total} 達成`}</strong></div>${s.targets.length?`<div class="routine-day-detail-list">${s.targets.map(r=>{const done=routineDoneOn(r,key);return`<div class="routine-day-detail-row ${future?"planned":done?"done":"missed"}"><span aria-hidden="true">${future?"○":done?"✓":"×"}</span><strong>${esc(r.title)}</strong><small>${esc(routineRuleLabel(r))} / ${esc(routineSlotLabel(r.slot))}</small></div>`}).join("")}</div>`:'<p class="routine-day-detail-empty">この日は対象のルーティンタスクがありません。</p>'}`}
function renderRoutineCalendar(){
  const routines=[...data.routines];if(routineCalendarFilterId!=="all"&&!routines.some(r=>r.id===routineCalendarFilterId))routineCalendarFilterId="all";
  E.routineCalendarSelect.innerHTML=`<option value="all">すべてのルーティン</option>`+routines.map(r=>`<option value="${r.id}">${esc(r.title)}</option>`).join("");E.routineCalendarSelect.value=routineCalendarFilterId;
  const y=routineCalCursor.getFullYear(),m=routineCalCursor.getMonth(),today=getRoutineLogicalDate(),days=new Date(y,m+1,0).getDate();E.routineCalendarMonthLabel.textContent=`${y}年 ${m+1}月`;E.routineCalendarGrid.innerHTML="";
  let monthTargets=0,monthDone=0,perfectDays=0;for(let day=1;day<=days;day++){const key=localDate(new Date(y,m,day));if(key>today)continue;const s=routineDayReflection(key);monthTargets+=s.total;monthDone+=s.doneCount;if(s.total&&s.doneCount===s.total)perfectDays++}const streaks=routineReflectionStreaks();E.routineMonthlyRate.textContent=(monthTargets?Math.round(monthDone/monthTargets*100):0)+"%";E.routinePerfectDays.textContent=perfectDays+"日";E.routineCurrentFullStreak.textContent=streaks.current+"日";E.routineLongestFullStreak.textContent=streaks.longest+"日";
  if(!routines.length){E.routineCalendarGrid.innerHTML='<div class="routine-calendar-empty">ルーティンタスクを追加すると、ここに月間振り返りが出ます。</div>';renderRoutineDayDetail();return}
  const first=new Date(y,m,1),start=new Date(y,m,1-first.getDay());for(let i=0;i<42;i++){const d=new Date(start);d.setDate(start.getDate()+i);const k=localDate(d),inMonth=d.getMonth()===m,future=k>today,s=routineDayReflection(k),level=!s.total?"none":!s.doneCount?"zero":s.doneCount===s.total?"full":"partial",cell=document.createElement("button");cell.type="button";cell.className=`routine-calendar-day ${level}${inMonth?"":" outside"}${k===today?" today":""}${k===routineReflectionDate?" selected":""}${future?" future":""}`;cell.innerHTML=`<span class="routine-calendar-date">${d.getDate()}</span><span class="routine-calendar-ratio">${s.total?`${s.doneCount}/${s.total}`:"—"}</span>`;cell.setAttribute("aria-label",`${k}：${s.total?`${s.doneCount}/${s.total}達成`:"対象なし"}`);cell.onclick=()=>{routineReflectionDate=k;renderRoutineCalendar()};E.routineCalendarGrid.appendChild(cell)}renderRoutineDayDetail()
}
...
function searchText(value){return String(value??"").normalize("NFKC").toLocaleLowerCase("ja-JP")}
function collectGlobalSearchResults(query){
  const q=searchText(query).trim();if(!q)return[];const results=[],add=(kind,id,label,detail,haystack,extra={})=>{if(searchText(haystack).includes(q))results.push({kind,id,label,detail,...extra})};
  data.boards.forEach(b=>{add("board",b.id,b.name,"ボード",b.name);b.sections.forEach(s=>s.cards.forEach(c=>add("card",c.id,c.title||"無題",`${b.name} / ${s.name}`,[c.title,c.content,c.link,...(c.tags||[])].join(" "),{boardId:b.id,sectionId:s.id}))) });
  data.quickMemos.forEach(m=>add("memo",m.id,m.title||"無題","メモ",`${m.title} ${m.content}`));
  data.routines.forEach(r=>add("routine",r.id,r.title,`${routineRuleLabel(r)} / ${routineSlotLabel(r.slot)}`,r.title));
  data.goalTowers.forEach(g=>add("goal",g.id,g.name,builderDisplayName(),`${g.name} ${g.note} ${(g.blocks||[]).map(b=>b.title).join(" ")}`));
  data.achievedGoals.forEach(g=>add("achievedGoal",g.id,g.name,"達成した目標",`${g.name} ${g.note} ${(g.blocks||[]).map(b=>b.title).join(" ")}`));
  return results
}
function globalSearchKindLabel(kind){return({board:"📁 ボード",card:"🗂 カード / タスク",memo:"📝 メモ",routine:"🔁 ルーティンタスク",goal:"🧭 目標",achievedGoal:"🏆 達成した目標"}[kind]||kind)}
function renderGlobalSearch(){const q=E.globalSearchInput.value,all=collectGlobalSearchResults(q),shown=all.slice(0,80);E.globalSearchSummary.textContent=q.trim()?`${all.length}件${all.length>shown.length?`（先頭${shown.length}件を表示）`:""}`:"検索語を入力してください。";E.globalSearchResults.innerHTML=shown.length?shown.map((x,i)=>`<button class="global-search-result" type="button" data-search-index="${i}"><span class="global-search-kind">${globalSearchKindLabel(x.kind)}</span><strong>${esc(x.label)}</strong><small>${esc(x.detail||"")}</small></button>`).join(""):q.trim()?'<p class="global-search-empty">一致する項目はありません。</p>':"";qa("[data-search-index]").forEach(button=>button.onclick=()=>openGlobalSearchResult(shown[Number(button.dataset.searchIndex)]))}
function openGlobalSearch(){E.globalSearchInput.value="";renderGlobalSearch();E.globalSearchModal.classList.remove("hidden");closeSide();setTimeout(()=>E.globalSearchInput.focus(),30)}
function openGlobalSearchResult(item){if(!item)return;closeModal("globalSearchModal");if(item.kind==="board"){data.selectedBoardId=item.id;showTransient("board");return}if(item.kind==="card"){data.selectedBoardId=item.boardId;showTransient("board");setTimeout(()=>document.querySelector(`[data-card-id="${CSS.escape(item.id)}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}),50);return}if(item.kind==="memo"){data.selectedQuickMemoId=item.id;showTransient("memo");return}if(item.kind==="routine"){data.selectedRoutineCalendarId=item.id;showTransient("routine");setTimeout(()=>document.querySelector(`[data-routine-id="${CSS.escape(item.id)}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}),50);return}if(item.kind==="goal"){data.selectedGoalId=item.id;showTransient("builder");return}if(item.kind==="achievedGoal"){showTransient("achievedGoals");setTimeout(()=>document.querySelector(`[data-achieved-goal-id="${CSS.escape(item.id)}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}),50)}}
...
function openSectionModal(id=null){const b=board(),s=id?b?.sections.find(x=>x.id===id):null;if(!b||id&&!s)return;editingSectionId=s?.id||null;E.sectionModalTitle.textContent=s?"区分を編集":"区分を追加";E.sectionNameInput.value=s?.name||"";E.sectionHomeInput.checked=!!s?.showOnHome;E.sectionModal.classList.remove("hidden");setTimeout(()=>E.sectionNameInput.focus(),20)}
function addSection(name,showOnHome=false){const b=board(),label=String(name||"").trim();if(!b||!label)return;b.sections.push({id:uid(),name:label,showOnHome:!!showOnHome,cards:[]});touchBoard(b);save();renderAll();dopaAction("AREA BUILD!!",label);standardAction("区分を追加",label,"save")}
function updateSection(id,name,showOnHome){const b=board(),s=b?.sections.find(x=>x.id===id),label=String(name||"").trim();if(!s||!label)return;s.name=label;s.showOnHome=!!showOnHome;touchBoard(b);save();renderAll()}
function saveSectionModal(){const name=E.sectionNameInput.value.trim();if(!name)return alert("区分名を入力してください。");if(editingSectionId)updateSection(editingSectionId,name,E.sectionHomeInput.checked);else addSection(name,E.sectionHomeInput.checked);closeModal("sectionModal")}
function reorderSection(sourceId,targetId){const b=board(),from=b?.sections.findIndex(x=>x.id===sourceId),to=b?.sections.findIndex(x=>x.id===targetId);if(!b||from<0||to<0||from===to)return;const [moved]=b.sections.splice(from,1);b.sections.splice(to,0,moved);touchBoard(b);save();renderBoard()}
function moveSection(id,direction){const b=board(),index=b?.sections.findIndex(x=>x.id===id),next=index+direction;if(!b||index<0||next<0||next>=b.sections.length)return;reorderSection(id,b.sections[next].id)}
function deleteSection(id){const b=board(),s=b?.sections.find(x=>x.id===id);if(!s||!confirm(`「${s.name}」を削除しますか？\n中のタスクは未分類へ移動します。`))return;let fallback=b.sections.find(x=>x.id!==id&&x.name==="未分類");if(s.cards.length&&!fallback){fallback={id:uid(),name:"未分類",showOnHome:false,cards:[]};b.sections.unshift(fallback)}if(fallback)fallback.cards.push(...s.cards);b.sections=b.sections.filter(x=>x.id!==id);if(!b.sections.length)b.sections.push({id:uid(),name:"未分類",showOnHome:false,cards:[]});touchBoard(b);save();renderAll();feedbackAction("区分を削除",s.name,"delete")}
function deleteBoard(){const b=board();if(!b)return;if(data.boards.length===1)return alert("最後のボードは削除できません。");if(!confirm(`「${b.name}」をゴミ箱へ移動しますか？`))return;const label=b.name,entry=trashAdd("board",b,{},b.name);data.boards=data.boards.filter(x=>x.id!==b.id);data.selectedBoardId=data.boards[0].id;save();renderAll();feedbackAction("ボードを削除",`${label} をゴミ箱へ移動`,"delete");offerTrashUndo(entry,label)}
...
function renderBoard(){const b=board();if(!b)return;E.currentBoardTitle.textContent=b.name;updateBoardPin();const tags=[...new Set(b.sections.flatMap(s=>s.cards.flatMap(c=>c.tags)))].sort(),old=E.tagFilter.value;E.tagFilter.innerHTML='<option value="">すべてのタグ</option>'+tags.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join("");if(tags.includes(old))E.tagFilter.value=old;E.sectionBoard.innerHTML="";b.sections.forEach(s=>E.sectionBoard.appendChild(renderSection(s)));selectionUI()}
function clearBoardDragIndicators(){qa(".board-section.section-drag-over,.card-grid.card-drag-over,.memo-card.card-drop-before").forEach(x=>x.classList.remove("section-drag-over","card-drag-over","card-drop-before"))}
function renderSection(s){const b=board(),index=b.sections.indexOf(s),wrap=document.createElement("section");wrap.className="board-section";wrap.dataset.sectionId=s.id;wrap.ondragover=e=>{if(!dragSection)return;e.preventDefault();clearBoardDragIndicators();wrap.classList.add("section-drag-over")};wrap.ondragleave=e=>{if(!wrap.contains(e.relatedTarget))wrap.classList.remove("section-drag-over")};wrap.ondrop=e=>{if(!dragSection)return;e.preventDefault();e.stopPropagation();const source=dragSection;clearBoardDragIndicators();reorderSection(source,s.id)};const head=document.createElement("div");head.className="section-header";const titleWrap=document.createElement("div");titleWrap.className="section-title-wrap";const handle=document.createElement("button");handle.type="button";handle.className="section-drag-handle";handle.textContent="⠿";handle.title="区分をドラッグして並び替え";handle.draggable=innerWidth>820;handle.ondragstart=e=>{e.stopPropagation();dragSection=s.id;e.dataTransfer.effectAllowed="move";wrap.classList.add("dragging")};handle.ondragend=()=>{dragSection=null;wrap.classList.remove("dragging");clearBoardDragIndicators()};const h=document.createElement("h3");h.className="section-title";h.textContent=s.name;titleWrap.append(handle,h);if(s.showOnHome){const badge=document.createElement("span");badge.className="section-home-badge";badge.textContent="Home";titleWrap.appendChild(badge)}const acts=document.createElement("div");acts.className="section-actions";const add=document.createElement("button");add.type="button";add.textContent="＋ Task";add.onclick=()=>openCardModal(null,b.id,s.id);const menu=document.createElement("details");menu.className="section-menu";const summary=document.createElement("summary");summary.textContent="…";summary.setAttribute("aria-label",`${s.name}の区分メニュー`);const panel=document.createElement("div");panel.className="section-menu-panel";const home=document.createElement("label");home.className="section-home-toggle";home.innerHTML=`<span>Homeに表示</span><input type="checkbox" ${s.showOnHome?"checked":""}>`;home.querySelector("input").onchange=e=>updateSection(s.id,s.name,e.target.checked);const rename=document.createElement("button");rename.type="button";rename.textContent="名前・設定を編集";rename.onclick=()=>openSectionModal(s.id);const up=document.createElement("button");up.type="button";up.textContent="↑ 上へ";up.disabled=index===0;up.onclick=()=>moveSection(s.id,-1);const down=document.createElement("button");down.type="button";down.textContent="↓ 下へ";down.disabled=index===b.sections.length-1;down.onclick=()=>moveSection(s.id,1);const del=document.createElement("button");del.type="button";del.className="section-delete-button";del.textContent="区分を削除";del.onclick=()=>deleteSection(s.id);panel.append(home,rename,up,down,del);menu.append(summary,panel);acts.append(add,menu);head.append(titleWrap,acts);const grid=document.createElement("div");grid.className="card-grid";grid.ondragover=e=>{if(!dragCard)return;e.preventDefault();e.stopPropagation();clearBoardDragIndicators();grid.classList.add("card-drag-over")};grid.ondragleave=e=>{if(!grid.contains(e.relatedTarget))grid.classList.remove("card-drag-over")};grid.ondrop=e=>dropSection(e,s.id);const a=filterCards([...s.cards]);if(a.length)a.forEach(c=>grid.appendChild(renderCard(c,s.id)));else grid.innerHTML='<div class="empty-section">ここにタスクを置けます。</div>';wrap.append(head,grid);return wrap}
function renderCard(c,sid){const el=document.createElement("article");el.className=`memo-card ${c.color||"white"}${c.selected?" selected":""}`;el.draggable=innerWidth>820&&E.sortSelect.value==="manual";el.dataset.cardId=c.id;el.ondragstart=e=>{e.stopPropagation();dragCard=c.id;e.dataTransfer.effectAllowed="move";el.classList.add("dragging")};el.ondragend=()=>{dragCard=null;el.classList.remove("dragging");clearBoardDragIndicators()};el.ondragover=e=>{if(!dragCard||dragCard===c.id)return;e.preventDefault();e.stopPropagation();clearBoardDragIndicators();el.classList.add("card-drop-before")};el.ondragleave=()=>el.classList.remove("card-drop-before");el.ondrop=e=>dropCard(e,sid,c.id);const head=document.createElement("div");head.className="card-head";if(c.type==="task"){const chk=document.createElement("input");chk.type="checkbox";chk.checked=c.selected;chk.className="card-select";chk.onchange=()=>{c.selected=chk.checked;c.updatedAt=Date.now();save();renderBoard()};head.appendChild(chk)}else{const i=document.createElement("span");i.className="card-type";i.textContent=({memo:"📝",idea:"💡",link:"🔗",image:"🖼"}[c.type]||"📝");head.appendChild(i)}const title=document.createElement("h3");title.className="card-title";title.textContent=(c.pinned?"📌 ":"")+(c.title||"無題");head.appendChild(title);el.appendChild(head);if(c.image){const img=document.createElement("img");img.className="card-image image-preview-trigger";img.alt=`${c.title||"カード"}の画像`;img.draggable=false;setImageSource(img,c.image);img.onclick=e=>{e.stopPropagation();openImageLightbox(c.image,img.alt)};el.appendChild(img)}if(c.content){const d=document.createElement("div");d.className="card-content";d.textContent=c.content;el.appendChild(d)}if(c.link){const a=document.createElement("a");a.className="card-link";a.href=c.link;a.target="_blank";a.rel="noopener";a.textContent=c.link;el.appendChild(a)}const meta=document.createElement("div");meta.className="meta-row";if(c.start||c.due){const d=document.createElement("span");d.className="due-chip"+(overdue(c)?" overdue":"")+(beforeStart(c)?" upcoming":"");d.textContent=periodLabel(c);meta.appendChild(d)}if(c.repeat?.type&&c.repeat.type!=="none"){const r=document.createElement("span");r.className="repeat-chip";r.textContent=repeatLabel(c.repeat);meta.appendChild(r)}if(meta.childNodes.length)el.appendChild(meta);if(c.tags.length){const t=document.createElement("div");t.className="tag-row";c.tags.forEach(x=>{const s=document.createElement("span");s.className="tag-chip";s.textContent="#"+x;t.appendChild(s)});el.appendChild(t)}const foot=document.createElement("div");foot.className="card-footer";const pin=document.createElement("button");pin.className="pin-card-button"+(c.pinned?" pinned":"");pin.textContent=c.pinned?"★ ピン":"☆ ピン";pin.onclick=()=>{c.pinned=!c.pinned;c.updatedAt=Date.now();touchBoard(board());save();renderBoard()};const edit=document.createElement("button");edit.textContent="編集 / 移動";edit.onclick=()=>openCardModal(c.id);const del=document.createElement("button");del.className="delete-card";del.textContent="削除";del.onclick=()=>deleteCard(c.id);foot.append(pin,edit,del);el.appendChild(foot);return el}
function dropSection(e,sid){e.preventDefault();e.stopPropagation();if(!dragCard)return;const id=dragCard,f=findCard(id),target=board().sections.find(s=>s.id===sid);if(!f||!target)return;f.s.cards=f.s.cards.filter(c=>c.id!==id);target.cards.push(f.c);f.c.updatedAt=Date.now();touchBoard(board());clearBoardDragIndicators();save();renderBoard()}
function dropCard(e,sid,targetId){e.preventDefault();e.stopPropagation();if(!dragCard||dragCard===targetId)return;const id=dragCard,f=findCard(id),t=board().sections.find(s=>s.id===sid);if(!f||!t)return;f.s.cards=f.s.cards.filter(c=>c.id!==id);const i=t.cards.findIndex(c=>c.id===targetId);t.cards.splice(Math.max(0,i),0,f.c);f.c.updatedAt=Date.now();touchBoard(board());clearBoardDragIndicators();save();renderBoard()}
function deleteCard(id){const f=findCard(id);if(!f||!confirm(`「${f.c.title}」をゴミ箱へ移動しますか？`))return;const label=f.c.title,entry=trashAdd("card",f.c,{boardId:f.b.id,boardName:f.b.name,sectionId:f.s.id,sectionName:f.s.name},f.c.title);f.s.cards=f.s.cards.filter(c=>c.id!==id);touchBoard(f.b);save();renderBoard();feedbackAction("カードを削除",`${label} をゴミ箱へ移動`,"delete");offerTrashUndo(entry,label)}
...
function completeCards(ids){
  if(!ids.length)return;let n=0;const beforeArchive=clone(data.archive),beforeRecent=clone(data.recent),beforeBoards=new Map();
  ids.forEach(id=>{const f=findCard(id);if(f&&!beforeBoards.has(f.b.id))beforeBoards.set(f.b.id,clone(f.b))});
  ...
  renderAll();
  if(n)offerLightUndo(`${n}件のタスクを完了しました`,()=>{data.archive=clone(beforeArchive);data.recent=clone(beforeRecent);data.boards=data.boards.map(b=>beforeBoards.has(b.id)?clone(beforeBoards.get(b.id)):b);save();renderAll()})
}
...
function clearLightUndo(){clearTimeout(undoTimer);undoTimer=null;pendingUndo=null;E.undoNotice?.classList.add("hidden")}
function offerLightUndo(message,action){clearLightUndo();pendingUndo={serial:mutationSerial,action};E.undoNoticeText.textContent=message;E.undoNotice.classList.remove("hidden");undoTimer=setTimeout(clearLightUndo,6500)}
function runLightUndo(){const item=pendingUndo;if(!item||item.serial!==mutationSerial)return clearLightUndo();undoRunning=true;pendingUndo=null;clearTimeout(undoTimer);try{item.action();feedbackAction("元に戻しました","直前の操作を取り消しました","undo")}finally{undoRunning=false;E.undoNotice.classList.add("hidden")}}
const imageStorage=()=>window.TaskImageStorage;
function isStorageImage(value){return!!imageStorage()?.isReference?.(value)}
function imageKey(value){return imageStorage()?.key?.(value)||""}
function storageRefCount(value,targetKey){if(!value||typeof value!=="object")return 0;if(isStorageImage(value))return imageKey(value)===targetKey?1:0;return(Array.isArray(value)?value:Object.values(value)).reduce((sum,item)=>sum+storageRefCount(item,targetKey),0)}
function setImageSource(img,value){const helper=imageStorage();if(helper?.setElementSource)return void helper.setElementSource(img,value);if(typeof value==="string")img.src=value}
async function imageValueForFile(file,fallback,area,ownerId){
  const helper=imageStorage();if(!helper?.canUpload?.())return fallback;
  try{return await helper.upload(file,area,ownerId)||fallback}catch(error){console.error(error);toast("画像は端末へ保存|クラウド保存できませんでした");return fallback}
}
function storageRefsNotIn(value,remaining){const helper=imageStorage();if(!helper)return[];const keep=new Set(helper.referencesIn(remaining).map(helper.key));return helper.referencesIn(value).filter(ref=>!keep.has(helper.key(ref)))}
async function removeStorageRefs(refs,{quiet=false}={}){
  if(!refs?.length)return true;
  try{await imageStorage().removeMany(refs);return true}catch(error){console.error(error);if(!quiet)alert("Storage画像を削除できませんでした。ログイン状態と通信環境を確認してください。\nデータは削除していません。");return false}
}
async function openImageLightbox(source,alt="画像"){
  if(!source)return;let src="";
  try{src=await(imageStorage()?.resolve?.(source)??source)}catch(error){console.error(error);return toast("画像を表示できません|ログイン状態と通信環境を確認してください")}
  if(!src)return;lightboxPreviousFocus=document.activeElement;E.imageLightboxImage.src=src;E.imageLightboxImage.alt=alt;E.imageLightbox.classList.remove("hidden");document.body.classList.add("image-lightbox-open");E.imageLightboxClose.focus()
}
function closeImageLightbox(){if(E.imageLightbox.classList.contains("hidden"))return;E.imageLightbox.classList.add("hidden");E.imageLightboxImage.removeAttribute("src");document.body.classList.remove("image-lightbox-open");lightboxPreviousFocus?.focus?.();lightboxPreviousFocus=null}
...
function builderBreakBlast(title,sub="",level="block"){
  ...
  const bits=isGoal?(mobileFx?58:110):(mobileFx?18:34);
  ...
  const streaks=isGoal?(mobileFx?18:34):(mobileFx?7:12);
  ...
  setTimeout(()=>{
    root.remove();
    document.body.classList.remove("builder-break-screen-shake","builder-goal-break-screen-shake");
  },isGoal?(mobileFx?3400:2950):(mobileFx?2100:1600));
}
...
function dopaAction(title,sub="",kind="action"){
  ...
  document.body.appendChild(root);const actionMs=innerWidth<=820?2300:1900;setTimeout(()=>root.remove(),actionMs);
}
...
function refreshCardSections(){const b=data.boards.find(b=>b.id===E.cardBoardInput.value)||board(),previous=E.cardSectionInput.value;E.cardSectionInput.innerHTML=(b?.sections||[]).map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join("");E.cardSectionInput.value=b?.sections.some(s=>s.id===previous)?previous:b?.sections[0]?.id||""}
function openCardModal(id=null,bid=null,sid=null){
  ...
  if(!b.sections.length)return alert("先に区分を追加してください。");
  editingCardId=id;editingCardDraftId=id||uid();editingImageData="";editingOriginalImage="";editingImageFile=null;qa(".weekday-input").forEach(x=>x.checked=false);
  if(id){
    ...
    editingImageData=c.image;editingOriginalImage=c.image;E.repeatTypeInput.value=c.repeat?.type||"none";E.repeatIntervalInput.value=c.repeat?.interval||1;
...
async function saveCard(){
  ...
  const start=E.cardStartInput.value,due=E.cardDueInput.value;
  if(start&&due&&start>due)return alert("着手可能日は期限日より前にしてください。");
  let finalImage=editingImageData;
  E.saveCardButton.disabled=true;
  try{
    if(editingImageFile)finalImage=await imageValueForFile(editingImageFile,editingImageData,"cards",editingCardDraftId);
    if(isStorageImage(editingOriginalImage)&&imageKey(editingOriginalImage)!==imageKey(finalImage)){
      const uses=storageRefCount(data,imageKey(editingOriginalImage));
      if(uses<=1&&!await removeStorageRefs([editingOriginalImage])){
        if(isStorageImage(finalImage))await removeStorageRefs([finalImage],{quiet:true});
        return
      }
    }
    const values={type:E.cardTypeInput.value,title:title||"無題",content,start,due,tags:E.cardTagsInput.value.split(",").map(x=>x.trim()).filter(Boolean),color:E.cardColorInput.value,link:E.cardLinkInput.value.trim(),image:finalImage,pinned:E.cardPinnedInput.value==="true",repeat:{type:E.repeatTypeInput.value,interval:Math.max(1,Number(E.repeatIntervalInput.value)||1),weekdays:qa(".weekday-input:checked").map(x=>Number(x.value))},updatedAt:Date.now()};
  if(editingCardId){
    ...
    const c={id:editingCardDraftId,...values,selected:false,createdAt:Date.now()};s.cards.push(c);b.updatedAt=Date.now();recent("card",c.id,c.title,`${b.name} / ${s.name}`,b.id)
  }
  ...
  }finally{E.saveCardButton.disabled=false}
}
function closeModal(id){
  $(id)?.classList.add("hidden");
  if(id==="settingsModal")document.body.classList.remove("settings-modal-open");
}
function readImage(file,cb){if(!file)return;try{imageStorage()?.validateFile?.(file)}catch(error){alert(error.message);return}if(file.size>2000000)alert("画像が大きめです。保存容量を圧迫する可能性があります。");const r=new FileReader();r.onload=()=>cb(r.result);r.onerror=()=>alert("画像を読み込めませんでした。");r.readAsDataURL(file)}
function imagePreview(){E.imagePreviewWrap.classList.toggle("hidden",!editingImageData);if(editingImageData)setImageSource(E.imagePreview,editingImageData);else E.imagePreview.removeAttribute("src")}
function addFreeImageFile(file){
  const page=freePage();if(!page)return;
  readImage(file,async fallback=>{
    const content=await imageValueForFile(file,fallback,"freeboard",page.id),before=freeSnap();
    page.items.push({id:uid(),type:"image",content,x:60,y:60,width:280,height:220,updatedAt:Date.now()});
    save();freeRecord(before);renderFreeboard()
  })
}
...
window.restoreArchive=id=>{const x=data.archive.find(x=>x.id===id);if(!x)return;const label=x.title,beforeBoards=clone(data.boards),beforeArchive=clone(data.archive);let b=data.boards.find(b=>b.id===x.boardId)||data.boards[0],s=b.sections.find(s=>s.id===x.sectionId)||b.sections.find(s=>s.name===x.sectionName)||b.sections[0];if(!s){s={id:uid(),name:"未分類",showOnHome:false,cards:[]};b.sections.push(s)}const c=clone(x);["boardId","boardName","sectionId","sectionName","completedAt"].forEach(k=>delete c[k]);c.selected=false;c.updatedAt=Date.now();s.cards.push(c);b.updatedAt=Date.now();data.archive=data.archive.filter(x=>x.id!==id);save();renderArchive();feedbackAction("完了記録をタスクに戻した",label,"restore");offerLightUndo(`${label} の完了を解除しました`,()=>{data.boards=clone(beforeBoards);data.archive=clone(beforeArchive);save();renderAll()})}
window.trashArchive=id=>{const x=data.archive.find(x=>x.id===id);if(!x)return;const label=x.title,entry=trashAdd("archive",x,{},"完了: "+x.title);data.archive=data.archive.filter(x=>x.id!==id);save();renderArchive();feedbackAction("完了記録をゴミ箱へ移動",label,"delete");offerTrashUndo(entry,label)}
...
function renderFreeboard(){
  ...
  p.items.forEach(i=>{
    ...
    if(i.type==="image"){const img=document.createElement("img");img.alt="自由帳画像";img.draggable=false;img.className="image-preview-trigger";setImageSource(img,i.content);img.onclick=e=>{e.stopPropagation();openImageLightbox(i.content,"自由帳画像")};body.appendChild(img)}else body.textContent=i.content;
...
function ensureMemo(){if(data.quickMemos.length)return;const now=Date.now(),m={id:uid(),title:"",content:"",pinned:false,createdAt:now,updatedAt:now};data.quickMemos.push(m);data.selectedQuickMemoId=m.id;save()}
function renderMemos(){ensureMemo();const cur=memo();E.quickMemoList.innerHTML="";data.quickMemos.forEach(m=>{const b=document.createElement("button");b.className="quick-memo-list-item"+(m.id===cur.id?" active":"");b.innerHTML=`<strong>${esc(m.title||"無題")}</strong><small>作成日：${new Date(m.createdAt||m.updatedAt).toLocaleString("ja-JP")}</small>${m.pinned?'<span class="quick-memo-list-pin">★</span>':""}`;b.onclick=()=>{data.selectedQuickMemoId=m.id;touchMemo(m);save();renderMemos()};E.quickMemoList.appendChild(b)});E.quickMemoTitle.value=cur.title;E.quickMemoContent.value=cur.content;E.quickMemoSavedLabel.textContent="自動保存";E.pinQuickMemoButton.textContent=cur.pinned?"★ ピン留め中":"☆ ピン留め"}
...
function trashMemo(){const m=memo();if(!m||!confirm("このメモをゴミ箱へ移動しますか？"))return;const label=m.title||"無題",entry=trashAdd("memo",m,{},label);data.quickMemos=data.quickMemos.filter(x=>x.id!==m.id);data.selectedQuickMemoId=data.quickMemos[0]?.id||null;save();renderMemos();feedbackAction("メモを削除",`${label} をゴミ箱へ移動`,"delete");offerTrashUndo(entry,label)}
...
function deleteGoal(){const g=goal();if(!g||!confirm(`「${g.name}」をゴミ箱へ移動しますか？`))return;const label=g.name,entry=trashAdd("goal",g,{},g.name);data.goalTowers=data.goalTowers.filter(x=>x.id!==g.id);data.selectedGoalId=data.goalTowers[0]?.id||null;save();renderBuilder();feedbackAction("目標を削除",`${label} をゴミ箱へ移動`,"delete");offerTrashUndo(entry,label)}
...
function renderAchieved(){const a=[...data.achievedGoals].sort((x,y)=>new Date(y.achievedAt)-new Date(x.achievedAt)),month=new Date().toISOString().slice(0,7),restoreLabel=data.settings.theme==="dopaboy"?"ブレイカーに戻す":"目標設計に戻す";E.achievedGoalCount.textContent=a.length;E.achievedGoalMonthCount.textContent=a.filter(x=>(x.achievedAt||"").startsWith(month)).length;E.achievedGoalList.innerHTML=a.length?a.map(x=>`<article class="achieved-goal-card" data-achieved-goal-id="${x.id}"><h3>🏆 ${esc(x.name)}</h3><div class="achieved-goal-meta">${x.due?`設定期限：${x.due}<br>`:""}達成日：${new Date(x.achievedAt).toLocaleString("ja-JP")}${x.note?`<br>${esc(x.note)}`:""}</div><div class="achieved-goal-blocks">${(x.blocks||[]).map(b=>`<span class="achieved-goal-block">💥 ${esc(b.title)}</span>`).join("")}</div><div class="achieved-goal-actions"><button class="secondary-button" onclick="restoreAchieved('${x.id}')">${restoreLabel}</button><button class="danger-outline-button" onclick="trashAchieved('${x.id}')">ゴミ箱へ</button></div></article>`).join(""):'<section class="panel"><p>達成した目標はまだありません。</p></section>'}
window.restoreAchieved=id=>{const x=data.achievedGoals.find(g=>g.id===id);if(!x)return;const original=clone(x),label=x.name,g=clone(x),destination=builderDisplayName();delete g.achievedAt;g.updatedAt=Date.now();data.goalTowers.push(g);data.selectedGoalId=g.id;data.achievedGoals=data.achievedGoals.filter(x=>x.id!==id);save();renderAchieved();feedbackAction(`達成目標を${destination}に戻した`,label,"restore");offerLightUndo(`${label} を${destination}に戻しました`,()=>{data.goalTowers=data.goalTowers.filter(x=>x.id!==g.id);data.achievedGoals.unshift(clone(original));data.selectedGoalId=data.goalTowers[0]?.id||null;save();renderAll()})};window.trashAchieved=id=>{const x=data.achievedGoals.find(g=>g.id===id);if(!x)return;const label=x.name,entry=trashAdd("achievedGoal",x,{},"達成: "+x.name);data.achievedGoals=data.achievedGoals.filter(x=>x.id!==id);save();renderAchieved();feedbackAction("達成目標をゴミ箱へ移動",label,"delete");offerTrashUndo(entry,label)}
function trashAdd(kind,payload,origin={},label=""){const entry={trashId:uid(),kind,label:label||payload.title||payload.name||"削除したもの",payload:clone(payload),origin:clone(origin),deletedAt:new Date().toISOString()};data.trash.unshift(entry);return entry}
function offerTrashUndo(entry,label){if(entry)offerLightUndo(`${label} をゴミ箱へ移動しました`,()=>{window.restoreTrash(entry.trashId);renderAll()})}
...
window.useTemplate=id=>{
  ...
  if(t.type==="board"){
    ...
    const b={id:uid(),name:name.trim(),pinned:false,createdAt:Date.now(),updatedAt:Date.now(),sections:clone(t.payload.sections||[]).map(s=>({id:uid(),name:s.name,showOnHome:!!s.showOnHome,cards:(s.cards||[]).map(c=>({...clone(c),id:uid(),due:"",selected:false,pinned:false,createdAt:Date.now(),updatedAt:Date.now()}))}))};
    if(!b.sections.length)b.sections=[{id:uid(),name:"未分類",showOnHome:false,cards:[]}];data.boards.push(b);data.selectedBoardId=b.id;touchBoard(b);save();show("board");return;
  }
...
window.toggleRoutineDone=id=>{const r=data.routines.find(x=>x.id===id);if(!r)return;const today=getRoutineLogicalDate(),wasDone=routineDoneOn(r,today),nowDone=!wasDone;routineMark(r,nowDone,today);touchRoutine(r);save();renderRoutine();renderPinned();if(data.view==="home")renderHome();if(data.view==="shortcuts")renderShortcuts();if(nowDone){dopaAction("DAILY CLEAR!!",r.title,"done");standardAction("ルーティンタスク完了",r.title,"done")}else feedbackAction("ルーティンタスクを未完了に戻す",r.title,"undo");offerLightUndo(`${r.title} を${nowDone?"完了":"未完了"}にしました`,()=>{const target=data.routines.find(x=>x.id===id);if(!target)return;routineMark(target,wasDone,today);touchRoutine(target);save();renderAll()})};
...
window.deleteRoutine=id=>{const r=data.routines.find(x=>x.id===id);if(!r||!confirm(`「${r.title}」をゴミ箱へ移動しますか？`))return;const label=r.title,entry=trashAdd("routine",r,{},r.title);data.routines=data.routines.filter(x=>x.id!==id);if(data.selectedRoutineCalendarId===id)data.selectedRoutineCalendarId=data.routines[0]?.id||null;save();renderRoutine();renderPinned();if(data.view==="shortcuts")renderShortcuts();feedbackAction("ルーティンタスクを削除",`${label} をゴミ箱へ移動`,"delete");offerTrashUndo(entry,label)};
...
async function shareData(){const f=backupFile();try{if(navigator.share&&navigator.canShare?.({files:[f]}))await navigator.share({files:[f],title:"タスク管理ンナー バックアップ",text:"タスク管理ンナー JSONデータ"});else{exportData();alert("共有シートが使えないため、バックアップを保存しました。")}}catch(e){if(e.name!=="AbortError")console.error(e)}}
...
function syncMobileSideButton(open=false){
  document.body.classList.toggle("mobile-nav-open",open);
  E.mobileMenuButton.setAttribute("aria-expanded",open?"true":"false");
  E.mobileMenuButton.textContent=open?"×":"☰";
}
function toggleSide(){
...
function bindSectionControls(){E.addSectionButton.onclick=()=>openSectionModal();E.saveSectionButton.onclick=saveSectionModal;E.sectionNameInput.onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();saveSectionModal()}};document.addEventListener("keydown",e=>{if(e.key==="Escape")closeModal("sectionModal")})}
buildDopaScatter();bind();bindSectionControls();bindEnhancements();save();renderAll();toggleRoutineRuleUI();
