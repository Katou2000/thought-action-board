...
window.restoreAchieved=id=>{const x=data.achievedGoals.find(g=>g.id===id);if(!x)return;const label=x.name,g=clone(x),destination=builderDisplayName();delete g.achievedAt;g.updatedAt=Date.now();data.goalTowers.push(g);data.selectedGoalId=g.id;data.achievedGoals=data.achievedGoals.filter(x=>x.id!==id);save();renderAchieved();feedbackAction(`達成目標を${destination}に戻した`,label,"restore")};window.trashAchieved=id=>{const x=data.achievedGoals.find(g=>g.id===id);if(!x)return;const label=x.name;trashAdd("achievedGoal",x,{},"達成: "+x.name);data.achievedGoals=data.achievedGoals.filter(x=>x.id!==id);save();renderAchieved();feedbackAction("達成目標をゴミ箱へ移動",label,"delete")}
