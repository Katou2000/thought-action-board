...
const DEFAULT_NAV=["home","calendar","routine","builder","freeboard","memo","boards","shortcuts","archive","achievedGoals","templates","trash"];
...
let editingCardId=null,editingCardDraftId=null,editingImageData="",editingOriginalImage="",editingImageFile=null,dragCard=null,dragBlock=null,editingFreeItemId=null,editingRoutineId=null,deferredInstall=null;
...
window.restoreArchive=id=>{const x=data.archive.find(x=>x.id===id);if(!x)return;const label=x.title,beforeBoards=clone(data.boards),beforeArchive=clone(data.archive);let b=data.boards.find(b=>b.id===x.boardId)||data.boards[0],s=b.sections.find(s=>s.id===x.sectionId)||b.sections.find(s=>s.name===x.sectionName)||b.sections[0];if(!s){s={id:uid(),name:"未分類",cards:[]};b.sections.push(s)}const c=clone(x);["boardId","boardName","sectionId","sectionName","completedAt"].forEach(k=>delete c[k]);c.selected=false;c.updatedAt=Date.now();s.cards.push(c);b.updatedAt=Date.now();data.archive=data.archive.filter(x=>x.id!==id);save();renderArchive();feedbackAction("完了記録をタスクに戻した",label,"restore");offerLightUndo(`${label} の完了を解除しました`,()=>{data.boards=clone(beforeBoards);data.archive=clone(beforeArchive);save();renderAll()})}
...
buildDopaScatter();bind();bindEnhancements();save();renderAll();toggleRoutineRuleUI();
