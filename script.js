...
function routineReflectionTarget(r,dateKey){if(!routineScheduledOn(r,dateKey))return false;const today=localDate();/* pausedAt履歴がない既存形式では、休止は今日以降だけ除外し、過去ログを失わない。 */return !(r.paused&&dateKey>=today&&!routineDoneOn(r,dateKey))}
...
function renderRoutineCalendar(){
  ...
  const y=routineCalCursor.getFullYear(),m=routineCalCursor.getMonth(),today=localDate(),days=new Date(y,m+1,0).getDate();E.routineCalendarMonthLabel.textContent=`${y}年 ${m+1}月`;E.routineCalendarGrid.innerHTML="";
