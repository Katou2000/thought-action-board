...
ローカル保存を正式な保存先として維持しながら、Supabaseへの手動同期を追加したリリースです。

■ 重要
- 保存キーは従来どおり `taskKanrinnerV1`。既存データをそのまま引き継ぎます。
- データはブラウザの localStorage に保存されます。
- PCとスマホは自動同期しません。設定画面の「クラウドへ保存」「クラウドから取得」で手動同期できます。
...
4. アプリの設定 → クラウド同期（手動）から新規登録またはログイン

Secret key / service_role keyはブラウザ用ファイルへ設定しないでください。
同期はボタン操作時だけ実行され、自動同期・Realtime・競合の自動マージは行いません。
