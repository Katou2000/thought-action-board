...
(()=>{
  ...
  function nowTaskInfo(card,today=todayKey()){
    ...
    const group=homeGroupFor(card);return group?{group:4,label:"ホーム区分",tone:"section",detail:group.name}:null
  }
  function currentTasks(){const unique=[...new Map(cards().filter(c=>c.type==="task").map(card=>[card.id,card])).values()];return unique.map(c=>({card:c,info:nowTaskInfo(c)})).filter(x=>x.info).sort((a,b)=>a.info.group-b.info.group||(a.card.due||"9999-99-99").localeCompare(b.card.due||"9999-99-99")||(b.card.pinned-a.card.pinned))}
  ...
  function renderHomeRoutines(){
    const routineKey=getRoutineLogicalDate(),stats=routineStats(routineKey),slotOrder={morning:0,day:1,night:2,any:3},items=[...stats.applicable].sort((a,b)=>(slotOrder[a.slot]??9)-(slotOrder[b.slot]??9)||(b.pinned-a.pinned)||a.title.localeCompare(b.title,"ja"));
    get("homeRoutineCount").textContent=`${stats.done.length} / ${stats.total}`;
    get("homeRoutineList").innerHTML=items.length?items.map(r=>`<label class="home-routine-item"><input type="checkbox" ${routineDoneOn(r,routineKey)?"checked":""} onchange="toggleRoutineDone('${r.id}')"><span><strong>${r.pinned?"📌 ":""}${esc(r.title)}</strong><small>${esc(routineSlotLabel(r.slot))}</small></span></label>`).join(""):'<div class="now-task-empty home-routine-empty"><strong>今日のルーティンはありません</strong><span>今日が対象のルーティンだけを表示します。</span></div>'
  }
  renderHome=function(){
  ...
  renderCalendar=function(){
    ...
    const quickItems=[...data.quickTasks.map(task=>({...task,legacy:false})),...data.quickTaskLog.filter(log=>!data.quickTasks.some(task=>task.id===log.id)).map(log=>({...log,completed:true,legacy:true}))];
    cards().filter(c=>c.type==="task"&&(c.plannedDate||c.start||c.due)).forEach(c=>{const add=(date,type)=>{if(!date)return;if(!map.has(date))map.set(date,[]);map.get(date).push({kind:"normal",c,type})};add(c.plannedDate,"planned");add(c.start,"start");add(c.due,"due")});quickItems.forEach(task=>{if(!task.plannedDate)return;if(!map.has(task.plannedDate))map.set(task.plannedDate,[]);map.get(task.plannedDate).push({kind:"light",task,type:"light"})});E.calendarGrid.innerHTML="";
    for(let i=0;i<42;i++){const date=new Date(gridStart);date.setDate(gridStart.getDate()+i);const key=localDate(date),events=map.get(key)||[],cell=document.createElement("div");cell.className="calendar-day"+(date.getMonth()!==m?" outside":"")+(key===todayKey()?" today":"")+(key===selectedDate?" selected":"")+(events.length?" has-events":"");
      const head=document.createElement("div");head.className="calendar-day-head";const number=document.createElement("button");number.type="button";number.className="calendar-date-number";number.textContent=date.getDate();number.title=`${key}の内容を見る`;number.onclick=e=>{e.stopPropagation();selectedDate=key;renderCalendar()};const add=document.createElement("button");add.type="button";add.className="calendar-add-button";add.textContent="＋";add.setAttribute("aria-label",`${key}にタスクを追加`);add.onclick=e=>{e.stopPropagation();selectedDate=key;window.openSharedCreateMenu(e,{source:"calendar-day",plannedDate:key});renderCalendar()};head.append(number,add);cell.appendChild(head);
      events.slice(0,3).forEach(item=>{const event=document.createElement("button");event.type="button";event.className=`calendar-task-dot${item.kind==="light"?` light${item.task.completed?" completed":""}`:""}`;if(item.kind==="light"){event.textContent=`${item.task.completed?"☑":"☐"} ${item.task.title}`;event.onclick=e=>{e.stopPropagation();selectedDate=key;renderCalendar()}}else{event.style.setProperty("--event-color",cardColors[item.c.color]||cardColors.white);event.textContent=(item.type==="planned"?"● ":item.type==="start"?"▶ ":"〆 ")+item.c.title;event.onclick=e=>{e.stopPropagation();openCardAny(item.c.id)}}cell.appendChild(event)});if(events.length>3){const more=document.createElement("span");more.className="calendar-more";more.textContent=`+${events.length-3}`;cell.appendChild(more)}cell.onclick=()=>{selectedDate=key;renderCalendar()};E.calendarGrid.appendChild(cell)}renderDay(selectedDate);if(data.view==="calendar")activeLocation=captureLocation()
  };
  renderDay=function(key){
    const normal=cards().filter(c=>c.type==="task"&&(c.plannedDate===key||c.start===key||c.due===key)),currentIds=new Set(data.quickTasks.map(task=>task.id)),light=[...data.quickTasks.filter(task=>task.plannedDate===key).map(task=>({...task,legacy:false})),...data.quickTaskLog.filter(log=>log.plannedDate===key&&!currentIds.has(log.id)).map(log=>({...log,completed:true,legacy:true}))];
    const normalRows=normal.map(c=>`<div class="calendar-detail-item${c.selected?" completed":""}"><label class="task-check-control"><input type="checkbox" ${c.selected?"checked":""} onchange="toggleTaskSelected('${c.id}',this.checked)"><span aria-hidden="true">${c.selected?"☑":"□"}</span></label><span class="item-main"><strong>${c.pinned?"📌 ":""}${esc(c.title)}</strong><small>${c.plannedDate===key?"● この日にやる":c.start===key&&c.due===key?"▶ 開始 / 〆":c.start===key?"▶ 期間開始":"〆 期限"} / ${esc(c.boardName)} / ${esc(c.sectionName)}</small></span><div class="header-actions"><button class="secondary-button" onclick="openCardAny('${c.id}')">開く</button>${c.selected?`<button class="success-button" onclick="completeOne('${c.id}')">完了</button>`:""}</div></div>`).join(""),lightRows=light.map(task=>`<div class="calendar-detail-item light${task.completed?" completed":""}"><button class="quick-task-check" type="button" ${task.legacy?"disabled":""} onclick="completeLightTask('${task.id}')" aria-label="${esc(task.title)}を${task.completed?"未完了に戻す":"完了"}">${task.completed?"☑":"□"}</button><span class="item-main"><strong>${esc(task.title)}</strong><small>軽量タスク / ${esc(data.boards.find(board=>board.id===task.boardId)?.name||"ボードなし")}</small></span></div>`).join("");
    E.calendarDayDetail.innerHTML=`<div class="calendar-detail-head"><h4>${key}</h4><button class="primary-button" type="button" onclick="openCalendarCreateMenu(event,'${key}')">＋ この日にタスク追加</button></div>${normalRows}${lightRows}${normalRows||lightRows?"":"<p>この日のタスクはありません。</p>"}`
  };
  ...
  function syncDopaThemeOption(){let option=E.themeSelect.querySelector('option[value="dopaboy"]');if(data.settings.dopaUnlocked&&!option){option=document.createElement("option");option.value="dopaboy";option.textContent="DOPA-BOY";E.themeSelect.appendChild(option)}else if(!data.settings.dopaUnlocked&&option)option.remove()}
  const baseApplyTheme=applyTheme;
  applyTheme=function(){syncDopaThemeOption();baseApplyTheme();document.body.dataset.tabSize=data.settings.tabSize;document.body.dataset.tabWidth=data.settings.tabWidth;const select=get("tabSizeSelect"),widthSelect=get("tabWidthSelect");if(select)select.value=data.settings.tabSize;if(widthSelect)widthSelect.value=data.settings.tabWidth;E.shortcutsButton.textContent="☆ ピン留め";E.shortcutsHelp.textContent="ピン留めした項目をまとめて確認。";E.homeHelper.textContent="今やることと全体の状況を確認。";E.calendarHelper.textContent="予定日・期間・期限を日付で確認。";get("restartDopaTutorialButton")?.classList.toggle("hidden",data.settings.theme!=="dopaboy");get("restartGoalTutorialButton")?.classList.toggle("hidden",data.settings.theme==="dopaboy");if(uxReady&&data.settings.theme==="dopaboy"&&!data.settings.dopaTutorialCompleted)setTimeout(()=>openTutorial("dopa"),80)};
  ...
  let dopaUnlockTapCount=0,dopaUnlockResetTimer=null;
  get("appVersionButton").onclick=()=>{if(data.settings.dopaUnlocked)return;dopaUnlockTapCount++;clearTimeout(dopaUnlockResetTimer);dopaUnlockResetTimer=setTimeout(()=>dopaUnlockTapCount=0,4000);if(dopaUnlockTapCount<5)return;dopaUnlockTapCount=0;clearTimeout(dopaUnlockResetTimer);data.settings.dopaUnlocked=true;save();syncDopaThemeOption();toast("DOPA MODE UNLOCKED.")};
